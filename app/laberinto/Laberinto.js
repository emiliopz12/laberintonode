...
/** @constant
    ...
    @default
*/
const DIFICULTAD = {FACIL: 3, DIFICIL: 4};

 /**
 * Modulos requeridos
 */	 
const matrix = require('../laberinto/Matrix'); 
const pArray = require('../laberinto/PArray'); 
const myMath = require('../laberinto/MyMath'); 

const PArray = pArray.PArray; 
const Matrix = matrix.Matrix;
const MyMath = myMath.MyMath;
...
obtenerVecinos (matrix, i, j) { 
      return [{i: i - 1, j: j, direccion: UP_WALL}, {i: i + 1, j: j, direccion: DOWN_WALL}, {i: i, j: j - 1, direccion: LEFT_WALL}, {i: i, j: j + 1, direccion: RIGHT_WALL}]
...
let crearLaberinto = (matrix, i = 0, j = 0) => (
     ...
     new PArray(...this.obtenerVecinos(matrix, i, j)).shuffle().filter((vecino) =>
         matrix[vecino.i][vecino.j].murosActivos() >= obj.dificultad
      ).forEach( (cell) => (matrix[cell.i][cell.j].murosActivos() >= obj.dificultad) ? (matrix[i][j].eliminarMuro(cell.direccion), matrix[ cell.i ][cell.j].eliminarMuroContrario(cell.direccion), crearLaberinto(matrix, cell.i, cell.j)): 0
)) : 0 ) ;

this.crearMatrix(obj);
crearLaberinto( this.matrix, MyMath.RandomInt(this.matrix.length, 0), MyMath.RandomInt(this.matrix[0].length, 0) );
return this.matrix;
